...
from VIHE.models.utils.rotary_enc import apply_rotary_emb
...
class PreNorm(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        if exists(self.norm_context):
            ...
            if "context2" in kwargs:
                context2 = kwargs["context2"]
                normed_context2 = self.norm_context(context2)
...
class Attention(nn.Module):  # is all you need. Living up to its name.
    ...
    def forward(self, x, context=None, mask=None):
        ...
        out = self.to_out(out)
        return out #, save_attn
    
class RotaryAttention(nn.Module):  # is all you need. Living up to its name.
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.0):
    ...
    def forward(self, x, x_freq=None, context=None, context_freq=None, mask=None):
        h = self.heads

        q = self.to_q(x)
        context = default(context, x)
        context_freq = default(context_freq, x_freq)

        k, v = self.to_kv(context).chunk(2, dim=-1)
        q = apply_rotary_emb(x_freq, q)
        k = apply_rotary_emb(context_freq, k)
...
class MultiviewRotaryCrossAttention(nn.Module):  # is all you need. Living up to its name.
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.0):
    ...
    def forward(self, x, x_freq, context, context_freq, mask=None):
        h = self.heads
        num_img = context.shape[1]

        x = x.unsqueeze(1).repeat(1, num_img, 1, 1).flatten(0, 1)

        context = context.flatten(0, 1)
        # x_freq = x_freq.permute(0, 2, 1, 3).flatten(0, 1)
        # context_freq = context_freq.permute(0, 2, 1, 3).flatten(0, 1)
        ...
        # move num image to last dim
        sim = rearrange(sim, "(b v h) n d -> (b h) n (v d)", h=h, v=num_img)
        v = rearrange(v, "(b v h) n d -> (b h) (v n) d", h=h, v=num_img)
...
class MultiviewRotaryCrossAttention2(nn.Module):  # is all you need. Living up to its name.
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.0):
    ...
    def forward(self, x, x_freq, x_freq2, context, context_freq, context2=None, context_freq2=None, mask=None):
        h = self.heads
        num_img = context.shape[1]

        x = x.unsqueeze(1).repeat(1, num_img, 1, 1).flatten(0, 1)

        def get_attn_v(context, context_freq):
            context = context.flatten(0, 1)
            q = self.to_q(x)
            k, v = self.to_kv(context).chunk(2, dim=-1)
        ...
        attn, v = get_attn_v(context, context_freq)
        attn2, v2 = get_attn_v(context2, context_freq2)
