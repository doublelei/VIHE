...
class BoxRenderer:
    ...
    #     R_, T_, scale = get_cube_R_T(
    #         with_scale=True,
    #     )
    #     real_cameras = pickle.load(open('/mnt/home/v_wangweiyao/workspace/baidu/personal-code/VLM/RVT/rvt/data/real_cameras.pkl', 'rb'))
    #     R = real_cameras['R']
    #     T = real_cameras['T'].squeeze(-1) 
    #     import pdb; pdb.set_trace()
    #     FoV = torch.tensor([40.0, 40.0, 40.0, 60.0])

    #     if self._fix_cam is None:
    #         assert len(R.shape) == len(T.shape) + 1 == 3
    #         # import pdb; pdb.set_trace()
    #         self._fix_cam = FoVPerspectiveCameras(
    #             device=self.device,
    #             R=R,
    #             T=T,
    #             fov= FoV,
